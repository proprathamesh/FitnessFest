...
export default function Card(props) {
    return (
        <div className='shadow-md shadow-gray-400 hover:shadow-lg hover:shadow-gray-400 overflow-hidden card h-[24rem] w-[17rem] relative '>
            ...
            <img className={`card-top h-[48%] hover:scale-[1.1] transition duration-500 w-full object-cover object-center`} src={props.cardImg} alt="" />
            <div className='overflow-hidden card-bottom h-[52%] w-full bg-black '>
                <div className="card-details pl-6 pt-6 pr-2">
                    <div className='flex gap-2'>
                        <h3 className='text-lg  text-white font-bannerfont '>Starting</h3>
                        <p> - {props.date}</p>
                    </div>
                    <h1 className='text-2xl text-pink-600 font-bannerfont font-bold h-16'>{props.competition}</h1>
                    <a class="border border-1 border-purple-700 py-1 px-2 mt-5 hover:bg-pink-600 hover:text-white hover:border-pink-600 bg-purple-700 transition duration-300" href="#" role="button">Register</a>
                </div>
            </div>
        </div>
    )
